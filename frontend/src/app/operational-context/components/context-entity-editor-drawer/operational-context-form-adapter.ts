...
const TYPE_FIELDS: Record<OperationalContextWritableType, OperationalContextFormSection[]> = {
  ...
  repository: [
    ...
    { title: 'Git identity and references', fields: [
      field('git', 'Git identity', 'repository-git', 'Enter the GitLab identity: provider, group, projectPath, optional project label, defaultBranch, URL and aliases.', 'projectPath resolves this catalogue entry to the GitLab project used by code tools, ownership resolution and repository discovery.', 'Structured Git fields. provider must be gitlab; projectPath is required and must be the provider-relative group/project path.', { required: true, example: 'Set provider gitlab, group crm, project contact-service and projectPath crm/contact-service.' }),
  ...
  process: [
    ...
    { title: 'Signals and advanced', fields: [
      ...
      field('relations', 'Relations', 'relations', 'Connect this process to adjacent processes, integrations or other catalogue entities using an explicit semantic edge.', 'Validated targets become graph edges and allow AI to continue into adjacent functional or technical context.', 'Guided relation rows. Choose a non-self catalogue target or a durable external target label.', { example: 'Add followed-by → process → CRM Contact Synchronization.' }),
...
export class OperationalContextFormAdapter {
  ...
  payload(type: OperationalContextWritableType, form: FormGroup<Record<string, FormControl<string>>>): OperationalContextPayload {
    const result: OperationalContextPayload = {};
    for (const field of this.fields(type)) {
