...
interface ConfigurationRenderRow {
  key: string;
  node: ConfigDriftViewerDiffNode;
  depth: number;
  changed: boolean;
  descendantChanged: boolean;
  annotations: ConfigDriftViewerDiffAnnotation[];
}

interface ConfigurationRenderDocument {
  key: string;
  document: ConfigDriftViewerDiffDocument;
...
interface ConfigurationRenderFile {
  key: string;
  file: ConfigDriftViewerDiffFile;
...
@Component({
  selector: 'app-config-drift-viewer-diff-renderer',
  templateUrl: './config-drift-viewer-diff-renderer.html',
  styleUrl: './config-drift-viewer-diff-renderer.scss'
})
export class ConfigDriftViewerDiffRendererComponent {
  private readonly elementRef = inject<ElementRef<HTMLElement>>(ElementRef);

  readonly projection = input.required<ConfigDriftViewerDiffProjection>();
  readonly annotations = input<ConfigDriftViewerDiffAnnotation[]>([]);
  readonly mode = input<ConfigDriftViewerMode>('BASIC');
  ...
  private readonly annotationsByDifferenceId = computed(() => {
    const index = new Map<string, ConfigDriftViewerDiffAnnotation[]>();
  ...
  protected fileLabel(file: ConfigDriftViewerDiffFile): string {
  ...
  protected formatValue(value: ConfigDriftViewerDiffValue): string {
  ...
  protected sameSides(node: ConfigDriftViewerDiffNode): boolean {
    return this.sameValue(node.source, node.target);
  }

  protected presenceDiff(node: ConfigDriftViewerDiffNode): boolean {
    return node.source.presence !== node.target.presence;
  }

  protected inlineValueDiff(node: ConfigDriftViewerDiffNode): InlineValueDiff | null {
    if (
  ...
  protected effectiveValueDiff(node: ConfigDriftViewerDiffNode): InlineValueDiff | null {
    if (
  ...
  protected leaf(node: ConfigDriftViewerDiffNode): boolean {
  ...
  protected collection(node: ConfigDriftViewerDiffNode): boolean {
    return ['MAP', 'LIST'].includes(node.source.type ?? '')
      || ['MAP', 'LIST'].includes(node.target.type ?? '');
  }

  protected chooseAnnotation(annotation: ConfigDriftViewerDiffAnnotation): void {
    this.referenceSelected.emit(annotation.sourceId);
  }

  protected annotationLabel(annotation: ConfigDriftViewerDiffAnnotation): string {
    return annotation.kind === 'FUNCTIONAL_IMPACT' ? 'Wpływ funkcjonalny' : 'Obserwacja AI';
  }

  private fileOrder(file: ConfigDriftViewerDiffFile): number {
  ...
  private renderFile(file: ConfigDriftViewerDiffFile, fileIndex: number): ConfigurationRenderFile {
  ...
  private annotationsFor(node: ConfigDriftViewerDiffNode): ConfigDriftViewerDiffAnnotation[] {
    if (this.mode() !== 'DEEP') {
      return [];
    }
    const result: ConfigDriftViewerDiffAnnotation[] = [];
  ...
  private changeCount(file: ConfigDriftViewerDiffFile): number {
  ...
  private nodeChangeCount(node: ConfigDriftViewerDiffNode): number {
    return node.differenceIds.length
      + node.children.reduce((total, child) => total + this.nodeChangeCount(child), 0);
  }

  private subtreeChanged(node: ConfigDriftViewerDiffNode): boolean {
