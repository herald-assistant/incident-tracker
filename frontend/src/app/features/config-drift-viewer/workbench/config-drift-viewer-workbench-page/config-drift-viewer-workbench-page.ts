...
import {
  ConfigDriftViewerBranchCoverage,
...
const ENDPOINT = '/api/config-drift-viewer/v1/workbench/preview';
const PAGE_SIZE = 100;
const EMPTY_OPTIONS: ConfigDriftViewerInputOptions = {
  modes: ['BASIC', 'DEEP'],
  branches: [],
  repositories: [],
  systems: []
...
@Component({
  selector: 'app-config-drift-viewer-workbench-page',
  imports: [ReactiveFormsModule],
  templateUrl: './config-drift-viewer-workbench-page.html',
  styleUrls: [
    '../../../source-console-layout.scss',
    './config-drift-viewer-workbench-page.scss'
  ]
})
export class ConfigDriftViewerWorkbenchPageComponent {
  ...
  readonly summary = signal<ConfigDriftViewerWorkbenchPreviewResponse | null>(null);
  ...
  readonly sourceDetail = signal<ConfigDriftViewerWorkbenchSourceResponse | null>(null);
  readonly configurationDiff =
    signal<ConfigDriftViewerWorkbenchConfigurationDiffResponse | null>(null);
  ...
  readonly mappingPage = signal<ConfigDriftViewerWorkbenchMappingPage | null>(null);
  readonly mappingChangedOnly = signal(true);
  readonly anonymizationPage =
    signal<ConfigDriftViewerWorkbenchAnonymizationPage | null>(null);
  readonly deepDetail = signal<ConfigDriftViewerWorkbenchDeepResponse | null>(null);
  readonly aiInput = signal<ConfigDriftViewerWorkbenchAiInputResponse | null>(null);
  readonly artifact = signal<ConfigDriftViewerWorkbenchArtifactResponse | null>(null);

  readonly form = new FormGroup({
    mode: new FormControl<ConfigDriftViewerMode>('BASIC', {
      nonNullable: true,
  ...
  selectMode(mode: ConfigDriftViewerMode): void {
  ...
  submit(event: Event): void {
    ...
    const payload: ConfigDriftViewerWorkbenchPreviewRequest = {
      mode: value.mode,
  ...
  private applyDefaults(options: ConfigDriftViewerInputOptions): void {
  ...
  private coverageRows(
    ...
    coverage: ConfigDriftViewerBranchCoverage | null
  ): SourceFileRow[] {
    if (!coverage) {
      return [];
    }
