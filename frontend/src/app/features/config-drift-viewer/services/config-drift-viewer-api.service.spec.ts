...
import { ConfigDriftViewerApiService } from './config-drift-viewer-api.service';

describe('ConfigDriftViewerApiService', () => {
  let service: ConfigDriftViewerApiService;
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(ConfigDriftViewerApiService);
  ...
  it('should load input options and encode DEEP preflight scope', () => {
    ...
    const options = http.expectOne('/api/config-drift-viewer/v1/input-options');
    ...
    const preflight = http.expectOne(
      (request) =>
        request.url === '/api/config-drift-viewer/v1/deep-preflight'
  ...
  it('should start, poll and import a verification result', () => {
    ...
    const pollRequest = http.expectOne('/api/config-drift-viewer/v1/jobs/job%2F1');
    expect(pollRequest.request.method).toBe('GET');
    pollRequest.flush({ jobId: 'job/1' });

    service.importResult({ schema: 'safe' }).subscribe();
    const importRequest = http.expectOne('/api/config-drift-viewer/v1/imports');
  ...
  it('should request a readonly Workbench preview with the exact selected scope', () => {
    ...
    const request = http.expectOne(
      '/api/config-drift-viewer/v1/workbench/preview'
    );
  ...
  it('should request Workbench snapshot details through dedicated lazy endpoints', () => {
    ...
    const base = '/api/config-drift-viewer/v1/workbench/preview/preview%2F1';
    ...
    service
      .getWorkbenchArtifact(previewId, 'config-drift-viewer/configuration-tree.yaml')
      .subscribe();
