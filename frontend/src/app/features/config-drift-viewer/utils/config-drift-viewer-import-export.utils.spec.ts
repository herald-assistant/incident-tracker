...
} from './config-drift-viewer-import-export.utils';

describe('config drift viewer import/export utils', () => {
  it('should build the versioned portable envelope and safe file name', () => {
    const job = portableJob();
    const envelope = buildConfigDriftViewerExportEnvelope(job, '2026-07-30T10:00:00Z');

    expect(envelope.schema).toBe('tdw.config-drift-viewer-export');
  ...
  it('should reject an unfinished job', () => {
    expect(() => buildConfigDriftViewerExportEnvelope(
      {
        ...portableJob(),
        status: 'RUNNING',
...
function portableJob(): ConfigDriftViewerJobStateSnapshot {
  return {
    jobId: 'job-1',
    mode: 'DEEP',
...
function portableResult(): ConfigDriftViewerResult {
  return {
