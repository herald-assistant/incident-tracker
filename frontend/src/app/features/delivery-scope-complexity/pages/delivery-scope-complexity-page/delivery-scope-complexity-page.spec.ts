...
import { DeliveryScopeComplexityPageComponent } from './delivery-scope-complexity-page';

describe('DeliveryScopeComplexityPageComponent', () => {
  it('should explain the deterministic scoring algorithm at the bottom of the composer', async () => {
    ...
    expect(algorithm.textContent).toContain('Struktura i logika');
    expect(algorithm.textContent).toContain('Dystrybucja zmiany');
    expect(algorithm.textContent).toContain('25%');
    expect(algorithm.textContent).toContain('scope = 0,5 + 1,5 × scopeSignal');
    expect(algorithm.textContent).toContain('wynik jednostki od 0 do 200');
    expect(algorithm.textContent).toContain('Confidence jest prezentowane osobno i nie zmienia wyniku');
  ...
  it('should reject a local run owned by another feature', async () => {
    ...
    expect(history.getRun).toHaveBeenCalledWith('job-1');
    expect(fixture.nativeElement.textContent).toContain('nie jest Delivery Scope Complexity');
  });
  ...
  it('should explain every overall result metric and its calculation', async () => {
    ...
    expect(tooltips[1]).toContain('Suma punktów z 1 ocenionych Delivery Units wynosi 8');
    expect(tooltips[1]).toContain('ważone wyniki sześciu wymiarów');
    expect(tooltips[2]).toContain('Średnia 8 / 200');
    expect(tooltips[3]).toContain('średnia confidence z 1 ocenionych jednostek wynosi 0,85');
    expect(tooltips[3]).toContain('HIGH od 0,80, MEDIUM od 0,60, LOW poniżej 0,60');
    expect(tooltips[4]).toContain('1 z 1 wszystkich jednostek');
    expect(tooltips[5]).toContain('EXCLUDED (0) i NOT_SCORABLE (0) wynosi 0');
    expect(tooltips[6]).toContain('Delivery Units ze statusem FAILED: 0');
  });
  ...
  it('should expose every merge request as a link above assessment dimensions', async () => {
    ...
    expect(details.querySelector('.unit-merge-requests')?.nextElementSibling?.classList.contains('dimension-panel'))
      .toBe(true);
    expect(details.querySelector('.dimension-panel')?.textContent).toContain('Dystrybucja zmiany');
  });
  ...
  it('should import a server-validated assessment and render it from the new history run', async () => {
    ...
    const imported = { ...completed, jobId: 'delivery-scope-import-1' };
    ...
    expect(fixture.componentInstance.job()?.jobId).toBe('delivery-scope-import-1');
    expect(fixture.nativeElement.textContent).toContain('CRM-1');
  });
...
async function createComponent(options: {
  queued?: DeliveryScopeComplexityJobStateSnapshot;
  completed?: DeliveryScopeComplexityJobStateSnapshot;
  localRun?: DeliveryScopeComplexityJobStateSnapshot;
  importResult?: DeliveryScopeComplexityJobStateSnapshot;
  localRunId?: string;
  localFeature?: string;
  ...
  const api = {
    startJob: vi.fn<(request: DeliveryScopeComplexityJobStartRequest) => Observable<DeliveryScopeComplexityJobStateSnapshot>>(
      () => of(queued).pipe(delay(0))
    ),
    getJob: vi.fn<(jobId: string) => Observable<DeliveryScopeComplexityJobStateSnapshot>>(
      () => of(completed).pipe(delay(0))
    ),
    importRun: vi.fn<(document: unknown) => Observable<DeliveryScopeComplexityJobStateSnapshot>>(
  ...
  const history = {
    getRun: vi.fn<(analysisId: string) => Observable<LocalAnalysisRunDetailResponse>>(() => of({
      analysisId: 'job-1',
      feature: options.localFeature ?? 'delivery-scope-complexity',
  ...
  await TestBed.configureTestingModule({
    imports: [DeliveryScopeComplexityPageComponent],
    providers: [
      { provide: DeliveryScopeComplexityApiService, useValue: api },
  ...
  const fixture = TestBed.createComponent(DeliveryScopeComplexityPageComponent);
  fixture.detectChanges();
  await fixture.whenStable();
  fixture.detectChanges();
...
function envelope(job: DeliveryScopeComplexityJobStateSnapshot): DeliveryScopeComplexityExportEnvelope {
  return {
...
function snapshot(
  status: string,
  points: number,
  units: DeliveryScopeComplexityJobStateSnapshot['units']
): DeliveryScopeComplexityJobStateSnapshot {
  ...
  return {
    ...
    aggregate: {
      ...
      averageComplexityScore: units.length ? points / units.length : 0,
...
function completedUnit(): DeliveryScopeComplexityJobStateSnapshot['units'][number] {
  return {
    unitId: 'DU-CRM-1',
    status: 'COMPLETED',
    issues: [{
...
function dimensionScore(score: number, scopeSignal: number, weight: number, points: number) {
  const scope = Math.round((0.5 + 1.5 * scopeSignal) * 10) / 10;
  return {
    score,
    scopeSignal,
    scope,
    scaledScore: Math.round(score * scope * 10) / 10,
    weight,
    points,
    evidence: [`dimension | artifact#section | evidence for score ${score}`]
  };
}
