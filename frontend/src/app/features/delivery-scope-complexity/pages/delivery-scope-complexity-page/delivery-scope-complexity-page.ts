...
import {
  DeliveryScopeAggregate,
  DeliveryScopeDimensionScore,
  DeliveryScopeDimensions,
  DeliveryScopeUnit,
  DeliveryScopeComplexityExportEnvelope,
...
type FilterOption = { value: string; label: string; issueCount: number; complexityPoints: number };
...
export class DeliveryScopeComplexityPageComponent implements OnDestroy {
  private readonly api = inject(DeliveryScopeComplexityApiService);
  ...
  readonly job = signal<DeliveryScopeComplexityJobStateSnapshot | null>(null);
  ...
  readonly visibleUnits = computed<DeliveryScopeUnit[]>(() => {
    this.filterRevision();
  ...
  readonly visibleAggregate = computed<DeliveryScopeAggregate | null>(() => {
    const job = this.job();
    if (!job) {
      return null;
    }
  ...
  protected async importRun(event: Event): Promise<void> {
    ...
    try {
      const document = await readJsonFile(
        file,
        'Plik nie zawiera poprawnego eksportu Delivery Scope Complexity.'
      );
      this.api
        .importRun(document)
        .pipe(
  ...
  protected filterOptionLabel(option: FilterOption): string {
    return `${option.label} · ${option.issueCount} issue · ${option.complexityPoints.toLocaleString('pl-PL')} pkt`;
  }

  protected filteredResultLabel(job: DeliveryScopeComplexityJobStateSnapshot): string {
  ...
  protected complexityPointsTooltip(aggregate: DeliveryScopeAggregate): string {
    return `Suma punktów z ${aggregate.assessedUnits} ocenionych Delivery Units wynosi ${aggregate.totalComplexityPoints.toLocaleString('pl-PL')}. `
      + 'Dla każdej jednostki backend sumuje ważone wyniki sześciu wymiarów po uwzględnieniu ich zakresu. '
      + 'Jednostki pominięte i błędne nie dodają punktów.';
  }

  protected averageComplexityTooltip(aggregate: DeliveryScopeAggregate): string {
    return aggregate.assessedUnits
      ? `Średnia ${aggregate.averageComplexityScore.toLocaleString('pl-PL')} / 200 to suma punktów podzielona przez ${aggregate.assessedUnits} ocenionych jednostek.`
      : 'Brak ocenionych jednostek, dlatego średni wynik wynosi 0.';
  }
  ...
  protected assessedUnitsTooltip(aggregate: DeliveryScopeAggregate): string {
    return `Delivery Units ze statusem COMPLETED, dla których AI zwróciło prawidłową ocenę, a backend `
      + `wyliczył wynik 0–200: ${aggregate.assessedUnits} z ${aggregate.totalUnits} wszystkich jednostek.`;
  }

  protected notAssessedUnitsTooltip(aggregate: DeliveryScopeAggregate): string {
    ...
    return `Suma jednostek EXCLUDED (${aggregate.excludedUnits}) i NOT_SCORABLE (${aggregate.notScorableUnits}) `
      ...
      + 'evidence. Żadne z nich nie zwiększa sumy punktów.';
  }

  protected failedUnitsTooltip(aggregate: DeliveryScopeAggregate): string {
    return `Delivery Units ze statusem FAILED: ${aggregate.failedUnits}. To przypadki niezakończone z powodu `
      + 'błędu przygotowania evidence, wywołania AI, walidacji lub parsowania odpowiedzi; nie zwiększają sumy punktów.';
  }
  ...
  protected dimensions(dimensions: DeliveryScopeDimensions): DimensionRow[] {
    return [
      { label: 'Nowość', value: dimensions.novelty },
      { label: 'Struktura i logika', value: dimensions.structuralAndLogic },
      { label: 'Reguły i inwarianty', value: dimensions.businessAndInvariants },
      { label: 'Odporność i testy', value: dimensions.robustnessAndTests },
      { label: 'Refaktoryzacja i architektura', value: dimensions.refactorAndArchitecture },
      { label: 'Dystrybucja zmiany', value: dimensions.distribution }
    ];
  }

  protected unitCostEstimate(unit: DeliveryScopeUnit): AnalysisAiCostEstimate | null {
    return estimateAnalysisAiCost(unit.usage);
  }

  protected unitCostTooltip(unit: DeliveryScopeUnit): string {
  ...
  protected unitWarnings(unit: DeliveryScopeUnit): string[] {
  ...
  protected unitHasAttention(unit: DeliveryScopeUnit): boolean {
  ...
  protected unitHasDetails(unit: DeliveryScopeUnit): boolean {
    return Boolean(
      unit.assessment
  ...
  protected unitAttentionLabel(unit: DeliveryScopeUnit): string {
  ...
  protected unitAttentionIcon(unit: DeliveryScopeUnit): string {
    return unit.status === 'NOT_SCORABLE' ? 'info' : 'warning';
  }

  protected unitAttentionClass(unit: DeliveryScopeUnit): string {
    return unit.status === 'NOT_SCORABLE'
      ? 'unit-attention-icon unit-attention-icon--info'
      : 'unit-attention-icon unit-warning-icon';
  }

  protected trackUnit(_: number, unit: DeliveryScopeUnit): string {
    return unit.unitId;
  }
  ...
  private startRequest(): DeliveryScopeComplexityJobStartRequest {
  ...
  private applyLocalRun(detail: LocalAnalysisRunDetailResponse): void {
    if (detail.feature !== 'delivery-scope-complexity') {
      this.jobError.set(`Lokalny run ${detail.analysisId} nie jest Delivery Scope Complexity.`);
      return;
    }
  ...
  private applyJobToForm(job: DeliveryScopeComplexityJobStateSnapshot): void {
  ...
  private unitTeamKeys(unit: DeliveryScopeUnit): string[] {
    return Array.from(new Set(
      unit.issues
        .map((issue) => issue.team)
        .filter((team): team is NonNullable<typeof team> => Boolean(team?.name))
  ...
  private unitAuthorKeys(unit: DeliveryScopeUnit): string[] {
    return Array.from(new Set(
      unit.mergeRequests
        .filter((mergeRequest) => mergeRequest.authorId !== null)
...
function jobFromEnvelope(value: unknown): DeliveryScopeComplexityJobStateSnapshot | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return null;
  }
  const envelope = value as Partial<DeliveryScopeComplexityExportEnvelope>;
  if (
    envelope.schema !== 'tdw.delivery-scope-complexity-export'
    || envelope.version !== 1
    || envelope.payload?.type !== 'delivery-scope-complexity'
    || envelope.payload.resultContract !== 'delivery-scope-complexity-v1'
...
function aggregateForUnits(units: DeliveryScopeUnit[]): DeliveryScopeAggregate {
  ...
  for (const unit of units) {
    ...
    totalComplexityPoints += unit.assessment.finalScore;
  ...
  const averageConfidence = confidenceValues.length
    ...
    : 0;
  return {
    totalComplexityPoints: round1(totalComplexityPoints),
    averageComplexityScore: assessedUnits ? round1(totalComplexityPoints / assessedUnits) : 0,
...
function aggregateUsage(units: DeliveryScopeUnit[]): DeliveryScopeAggregate['usage'] {
...
function filterOptions(
  units: DeliveryScopeUnit[],
  values: (unit: DeliveryScopeUnit) => { value: string; label: string }[]
): FilterOption[] {
  ...
  for (const unit of units) {
    ...
    for (const value of unitValues) {
      ...
      current.complexityPoints += unit.assessment?.finalScore ?? 0;
  ...
  return Array.from(byValue.entries())
    .map(([value, option]) => ({
      ...
      complexityPoints: round1(option.complexityPoints)
    }))
...
function countUnits(units: DeliveryScopeUnit[], status: string): number {
