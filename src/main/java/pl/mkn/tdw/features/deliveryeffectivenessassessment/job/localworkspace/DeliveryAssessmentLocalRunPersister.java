...
public class DeliveryAssessmentLocalRunPersister implements DeliveryAssessmentLocalRunPersistence {
    ...
    public static final String FEATURE = "delivery-effectiveness-assessment";
    ...
    @Override
    public void persistRunSnapshot(DeliveryEffectivenessAssessmentJobStateSnapshot snapshot) {
        if (!workspaceProperties.isEnabled()) {
            throw new IllegalStateException("Local workspace is disabled.");
        }
        var envelope = DeliveryEffectivenessAssessmentExportEnvelope.from(snapshot, exportTimestamp(snapshot));
    ...
    private Instant exportTimestamp(DeliveryEffectivenessAssessmentJobStateSnapshot snapshot) {
