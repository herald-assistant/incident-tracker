...
public class DeliveryScopeComplexityJobState {

    private final String jobId;
    private final DeliveryScopeComplexityJobStartRequest request;
    ...
    private final LinkedHashMap<String, DeliveryScopeUnitState> units = new LinkedHashMap<>();
    ...
    public DeliveryScopeComplexityJobState(
            String jobId,
    ...
    public synchronized void markUnitsReady(
            DeliveryScopeSourceResult source,
        ...
        deliveryUnits.forEach(unit -> {
            var unitState = new DeliveryScopeUnitState(unit);
    ...
    public synchronized void finalizeJob() {
        var allTerminal = units.values().stream().allMatch(DeliveryScopeUnitState::terminal);
        if (!allTerminal) {
            return;
        }
    ...
    public synchronized DeliveryScopeComplexityJobStateSnapshot snapshot() {
        var unitSnapshots = units.values().stream().map(DeliveryScopeUnitState::snapshot).toList();
    ...
    private DeliveryScopeAggregateResponse aggregate(List<DeliveryScopeUnitResponse> snapshots) {
        var totalPoints = 0.0;
        var scores = new ArrayList<Double>();
        var confidence = new ArrayList<Double>();
        for (var unit : snapshots) {
            if (unit.assessment() != null) {
                var score = unit.assessment().finalScore();
                totalPoints += score;
                scores.add(score);
        ...
        var averageScore = scores.stream().mapToDouble(Double::doubleValue).average().orElse(0);
        ...
        return new DeliveryScopeAggregateResponse(
                Math.round(totalPoints * 10.0) / 10.0,
    ...
    private AnalysisAiUsage usage(List<DeliveryScopeUnitResponse> snapshots) {
        var usages = snapshots.stream().map(DeliveryScopeUnitResponse::usage)
    ...
    private List<AnalysisJobStepResponse> steps(
            List<DeliveryScopeUnitResponse> snapshots,
    ...
    private List<AnalysisEvidenceSection> contextSections(List<DeliveryScopeUnitResponse> snapshots) {
    ...
    private DeliveryScopeUnitState unit(String unitId) {
    ...
    private int count(List<DeliveryScopeUnitResponse> snapshots, String status) {
