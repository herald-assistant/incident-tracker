...
@Component("deliveryScopeAiResponseParser")
...
public class DeliveryAiResponseParser {
    ...
    private DeliveryScopeDimensions dimensions(JsonNode node) {
        if (node == null || !node.isObject()) {
            throw new IllegalArgumentException("AI response did not contain dimensions.");
        }
        return new DeliveryScopeDimensions(
                requiredDimension(node, "novelty"),
                requiredDimension(node, "structuralAndLogic"),
                requiredDimension(node, "businessAndInvariants"),
                requiredDimension(node, "robustnessAndTests"),
                requiredDimension(node, "refactorAndArchitecture"),
                requiredDimension(node, "distribution")
        );
    }

    private DeliveryScopeDimension requiredDimension(JsonNode node, String name) {
        ...
        if (score == null || !score.isIntegralNumber() || !score.canConvertToInt()) {
            throw new IllegalArgumentException("AI response dimension " + name + " score is missing.");
        }
        var scopeSignal = value.get("scopeSignal");
        if (scopeSignal == null || !scopeSignal.isNumber()) {
            throw new IllegalArgumentException("AI response dimension " + name + " scopeSignal is missing.");
        }
        try {
            return new DeliveryScopeDimension(
                    score.asInt(),
                    scopeSignal.asDouble(),
                    optionalTextList(value.path("evidence"))
            );
        } catch (IllegalArgumentException exception) {
            throw new IllegalArgumentException("AI response dimension " + name + " is invalid: "
