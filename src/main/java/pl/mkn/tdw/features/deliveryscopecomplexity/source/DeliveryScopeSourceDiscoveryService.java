...
public class DeliveryScopeSourceDiscoveryService {
    ...
    public DeliveryScopeSourceResult discover(
            DeliveryScopeComplexityJobStartRequest request,
            DeliveryScopeSourceListener listener
    ) {
        ...
        var issues = new ArrayList<DeliveryScopeIssueSource>();
        ...
        return new DeliveryScopeSourceResult(
                search.effectiveJql(),
                search.total(),
                search.truncated(),
                issues,
    ...
    private IssueDiscoveryOutcome discoverIssue(
        ...
        try {
            ...
            return IssueDiscoveryOutcome.discovered(new DeliveryScopeIssueSource(
                    new DeliveryScopeIssue(candidate.issueKey(), doneAt, material, team, issueLimitations),
                    mergeRequests,
                    issueLimitations
            ));
        } catch (RuntimeException exception) {
    ...
    private DeliveryScopeTeam team(List<JiraIssueCustomField> customFields, List<String> limitations) {
    ...
    private void validateRange(DeliveryScopeComplexityJobStartRequest request) {
        if (!properties.isEnabled()) {
            throw new IllegalStateException("Delivery Scope Complexity is disabled.");
        }
        var days = ChronoUnit.DAYS.between(request.fromDate(), request.toDate()) + 1;
        if (days > properties.getMaxRangeDays()) {
            throw new IllegalArgumentException(
                    "Date range exceeds delivery-scope-complexity.max-range-days="
    ...
    private record IssueDiscoveryOutcome(
            DeliveryScopeIssueSource issue,
        ...
        static IssueDiscoveryOutcome discovered(DeliveryScopeIssueSource issue) {
