...
@Service
public class ConfigDriftViewerWorkbenchPreviewStore {
    ...
    public ConfigDriftViewerWorkbenchPreviewStore() {
        this(Clock.systemUTC(), DEFAULT_TTL, DEFAULT_MAX_ENTRIES);
    }

    ConfigDriftViewerWorkbenchPreviewStore(Clock clock, Duration ttl, int maxEntries) {
        this.clock = clock;
        this.ttl = ttl != null && !ttl.isNegative() && !ttl.isZero() ? ttl : DEFAULT_TTL;
        this.maxEntries = Math.max(1, maxEntries);
    }

    synchronized StoredPreview store(ConfigDriftViewerWorkbenchPreviewSnapshot snapshot) {
    ...
    ConfigDriftViewerWorkbenchPreviewSnapshot require(String previewId) {
        var now = clock.instant();
    ...
    record StoredPreview(
            String previewId,
            Instant expiresAt,
            ConfigDriftViewerWorkbenchPreviewSnapshot snapshot
    ) {
    }
